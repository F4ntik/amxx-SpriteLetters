...
1. Скомпилируйте проект (`build.bat`) либо возьмите готовые `*.amxx` из каталога `amxmodx/plugins/`.
...
Запустите `build.bat` (двойной клик или из PowerShell). Скрипт соберёт все `.sma` из `amxmodx/scripting/` и положит свежие `.amxx` в `amxmodx/plugins/`.

После обновления устанавливайте Core (`SprLetters`), Editor и Saver вместе: редактор использует общие функции изменения текста и настройки прокрутки из нового Core. Порядок в `plugins.ini`: Core, Saver, Editor, затем необязательный Gradient.
...
- Скорость `0` ставит строку на паузу. Её окно и позиция восстанавливаются сразу при загрузке; смена текста и сброс позиции работают и на паузе.
...
Плагин `[SprLett] Saver` создаёт необходимые каталоги и записывает слово в `addons/amxmodx/configs/plugins/SpriteLetters/Saves/<map>.json` по команде «Сохранить слово». Сохраняется снимок в момент команды. После перемещения, поворота, изменения текста или настройки бегущей строки снова нажмите «Сохранить слово». Завершение карты само по себе последние изменения не сохраняет.

Сообщение «Слово сохранено» появляется только после успешной записи и проверки файла. При ошибке редактор показывает уведомление и путь в консоли; неудачное удаление сохранения также оставляет слово на карте.

Новая версия сначала записывается в соседний `.tmp` и читается обратно для проверки. Предыдущая версия остаётся в `.bak`. При ошибке замены выполняется восстановление прежнего файла. Эта последовательность защищает от обычных ошибок записи, но не обещает сохранность при внезапном отказе диска или питания.

Повреждённый JSON, неверный корневой тип и следы незавершённой записи защищены от перезаписи. Если есть `.tmp`, либо основной файл отсутствует при наличии `.bak`, сохранение блокируется до восстановления. Сначала скопируйте основной файл, `.tmp` и `.bak` для сохранности, выберите корректную версию `<map>.json`, уберите обработанный `.tmp` из каталога и затем перезапустите карту. Во время работы карты не заменяйте сохранение вручную: обнаружив изменение, Saver остановит запись до повторной загрузки.

Полный исходный текст сохраняется независимо от включённой прокрутки. Автоматический ID выбирается свободным, а одинаковые ID, назначенные вручную, остаются доступны для группового управления.
...
**Где искать собранные плагины?** — В `amxmodx/plugins/`.
